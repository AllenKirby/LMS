...
const Home: React.FC = () => {
  ...
  const courses = useSelector(
    (state: { courses: TraineeCourses[] }) => state.courses
  );
  ...
  const { filterCoursesStatus, sortCourses, getCurrentTimeOfDay } =
    CoursesFunctions();
  ...
  const completedCourses = courses.filter(
    (course) => course.participant_status === "completed"
  );
  const now = new Date();
  const upcomingTrainings = externalTrainings.filter(
    (training) =>
      training?.training_details?.start_date ??
      new Date(training?.training_details?.start_date) > now
  );

  return (
    <section className="w-full h-full py-7 px-5 flex flex-row gap-5 bg-content-bg">
      <div className="w-full h-full overflow-y-auto flex flex-col gap-5 md:px-5">
        <section className="w-full flex flex-col gap-5">
          <article>
            <p className="text-p-sm font-medium text-f-gray">
              {getCurrentTimeOfDay()}
            </p>
            <h2 className="text-p-lg md:text-h-h5 font-medium text-f-black">
              Welcome back, {user?.user.first_name}!
            </h2>
          </article>
          <section className="flex flex-col md:flex-row space-y-3 md:space-x-4">
        ...
        <section className="flex flex-col h-full gap-3">
          <div className="flex justify-between">
            <p className="text-p-lg md:text-h-h6 font-medium text-f-dark">
              Course Overview
            </p>
            <select
              value={selectedFilters}
              onChange={(e) => setSelectedFilters(e.target.value)}
              className="text-f-dark px-2 py-1 bg-white shadow-md rounded-md"
            >
              Filter
          ...
          <nav className="flex flex-row w-full h-fit border-b text-nowrap">
          ...
          <div className="relative w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filterCoursesStatus(activeButtonCourse, courses).length > 0 ? (
              sortCourses(
                filterCoursesStatus(activeButtonCourse, courses),
                user,
                selectedFilters
              ).map((item) => (
