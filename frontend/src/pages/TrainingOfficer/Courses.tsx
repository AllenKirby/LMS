...
import ExternalTrainingForm from './ExternalTrainingForm'
        ...
        <div className="w-full flex items-center justify-between text-p-rg py-2">
          <nav>
            <ul className="flex text-p-sm">
              <button className={`px-3 py-2 flex items-center gap-1 ${activeSection === "/trainingofficer/courses/courseoverview"? 
                "border-2 rounded-md bg-c-grey-5 border-c-blue-50 font-medium text-c-blue-50" 
                : 
                "text-c-grey-50"}
              `}>{courseOverviewData ? <FaCircleCheck size={16} className="text-c-blue-50"/> : <FaCircleCheck size={16} className="text-c-grey-30"/>} <NavLink to='courseoverview'> Course Overview</NavLink></button>
              <button 
                disabled={!courseOverviewData}
                className={`px-3 py-2 flex items-center gap-1 ${activeSection === "/trainingofficer/courses/coursecontent" ? 
                  "border-2 rounded-md bg-c-grey-5 border-c-blue-50 font-medium text-c-blue-50" : 
                  "text-c-grey-50"} 
                  ${courseOverviewData ? "cursor-pointer" : "cursor-not-allowed"}`}
                onClick={() => handleNavigationRequest('/trainingofficer/courses/coursecontent')}
              >{courseOverviewData ? <FaCircleCheck size={16} className="text-c-blue-50"/> : <GoLock size={16}/>} Course Content</button>
              <button className={`px-3 py-2 flex items-center gap-1 ${activeSection === "/trainingofficer/courses/preview"? 
                "border-2 rounded-md bg-c-grey-5 border-c-blue-50 font-medium text-c-blue-50" 
                : 
                "text-c-grey-50"}
              `}><GoLock size={16}/><NavLink to='preview'>Preview</NavLink></button>
            </ul>
          </nav>
